...
app.innerHTML = `
  ...
  <aside id="correctionPanel" class="dictionaryPanel hidden">
    ...
    <textarea id="correctionText" class="dictSearch" style="min-height:220px; resize:vertical;"></textarea>
  ...
  document.querySelector('#segments').innerHTML = `
    ${(data.segments || []).map((seg, index) => `
      <article class="segment">
        <div class="segNum">
          Segment ${index + 1}
          ${currentLang === 'fr' && correctionAdminUnlocked ? `
            <button class="editFrBtn" data-type="segments" data-index="${index}">✏️ Corriger</button>
          ` : ''}
        </div>
...
function toggleCorrectionAdmin() {
  if (correctionAdminUnlocked) {
    ...
    renderDaf(currentDaf)
    alert('Mode correction désactivé.')
    return
  }
  ...
  renderDaf(currentDaf)
  alert('Mode correction activé.')
}

function installCorrectionButtons() {
...
function openCorrectionPanel(type, index) {
  ...
  const currentFile = localStorage.getItem('currentFile') || 'berakhot.json'
  const arr = currentData.dapim[currentDaf][type] || []
  ...
  document.querySelector('#correctionMeta').textContent =
    `${currentData.title} ${currentDaf} — ${type} ${index + 1}`
...
async function saveFrenchCorrection() {
  ...
  try {
    ...
    const arr = currentData.dapim[currentCorrectionTarget.daf][currentCorrectionTarget.type]
